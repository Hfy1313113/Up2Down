/* recognize.js —— 马形识别：从手绘笔画中定位躯干与四条腿，
...
const Recognize = (() => {
  ...
  function arcMidpoint(pts) {
    ...
    return pts[Math.floor(pts.length / 2)].slice();
  }
  ...
  function analyze(rawStrokes) {
    let strokes = rawStrokes
      .map(s => ({ points: (s.points || []).filter(p => isFinite(p[0]) && isFinite(p[1])) }))
    ...
    let legs = chosen.map(c => {
      ...
      // 髋 = 更靠近躯干中心水平线（y 较小）的一端；蹄 = 另一端
      let hip, foot;
      if (first[1] <= last[1]) { hip = first.slice(); foot = last.slice(); }
      else { hip = last.slice(); foot = first.slice(); }
    ...
    if (legs.length < 4) {
      ...
      for (const sx of slots) {
        ...
        const hip = [sx, torso.cy + torso.thick * 0.32];
        const knee = [sx + legLen * 0.06, hip[1] + legLen / 2];
        const foot = [sx + legLen * 0.1, hip[1] + legLen];
        legs.push({
          hip, knee, foot,
          L1: Math.hypot(knee[0] - hip[0], knee[1] - hip[1]),
          L2: Math.hypot(foot[0] - knee[0], foot[1] - knee[1]),
    ...
    // ---------- 4. 判断朝向并镜像 ----------
    // 头在躯干右侧 → 面向右；否则整体镜像（目标：面向 +x）
    if (head.x < torso.cx) {
      const mx = torso.cx;
      const mir = (p) => [2 * mx - p[0], p[1]];
    ...
    const scale = 120 / Math.max(torso.len, 1);   // 躯干统一为 120 单位长
    const toLocal = (p) => [(p[0] - torso.cx) * scale, (feetY - p[1]) * scale];
    // 本地系 y 向上为正（脚底 0，躯干中心 bodyH）
    ...
    const model = {
      torso: {
        ...
        angle: -torso.angle,
        len: torso.len * scale,
        // 手绘躯干笔画本身没有「厚度」信息，统一按躯干长度成比例生成桶状躯干
  ...
  function fallbackModel() {
    // 完全无法识别时的默认马（仍参与比赛，速度打折）
    const legs = [];
    for (let i = 0; i < 4; i++) {
      const x = -45 + i * 30;
      legs.push({
        hip: [x, 78], knee: [x + 3, 40], foot: [x + 6, 0],
  ...
  function analyzeParts(parts) {
    parts = parts || {};
    const legStrokes = (parts.legs || [])
      .map(s => ({ points: (s.points || []).filter(p => isFinite(p[0]) && isFinite(p[1])) }))
      .filter(s => s.points.length >= 3 && arcLength(s.points) > 6);
    const headStrokes = (parts.head || [])
      .map(s => ({ points: (s.points || []).filter(p => isFinite(p[0]) && isFinite(p[1])) }))
      .filter(s => s.points.length >= 3);
    const buttStrokes = (parts.butt || [])
      .map(s => ({ points: (s.points || []).filter(p => isFinite(p[0]) && isFinite(p[1])) }))
      .filter(s => s.points.length >= 3);
    ...
    // 头部：前端参考
    let headCx = null, headCy = null, headSize = H * 0.22;
    if (headStrokes.length) {
      let sx = 0, sy = 0, n = 0, hb = null;
    ...
    if (legs.length < 4) {
      ...
      for (const sx of slots) {
        ...
        const hip = [sx, torsoCy + thick * 0.32];
        const knee = [sx + legLen * 0.06, hip[1] + legLen / 2];
        const foot = [sx + legLen * 0.1, hip[1] + legLen];
        legs.push({
          hip, knee, foot,
  ...
  // ---------- 5. 标准化到本地坐标系（脚底 y=0，躯干中心 y=bodyH，面向 +x） ----------
  function normalize(raw) {
    const scale = 120 / Math.max(raw.torsoLen, 1);   // 躯干统一为 120 单位长
    const toLocal = (p) => [(p[0] - raw.cx) * scale, (raw.feetY - p[1]) * scale];
    ...
    const model = {
      ...
      tail: raw.tail ? { x: (raw.tail[0] - raw.cx) * scale, y: (raw.feetY - raw.tail[1]) * scale, found: true }
